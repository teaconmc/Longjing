...
EOM
  ...
  if [ `jq -r .mirror tmp.json` == 'false' ]; then
    ...
    aws s3api head-object $S3_OPTS --bucket 2021 --key "ci/extra/$NAME" >/dev/null && EXISTED=true
  fi
  aws s3api head-object $S3_OPTS --bucket 2021 --key "ci/extra/$NAME.asc" >/dev/null && SIGNED=true
  ...
  if [ ! $EXISTED ]; then
    ...
    if [ ! $? ]; then
      ...
      aws s3 cp $S3_OPTS $NAME "s3://2021/ci/extra/$NAME"
    fi
  fi
...
$PGP_PWD
EOM
    aws s3 cp $S3_OPTS "$NAME.asc" "s3://2021/ci/extra/$NAME.asc"
  fi
